...
while(string[space] == ' ') {
    if(string[space] == ' ')
    num_space++; //узнали сколько пробелов есть в строке
space--;
}
...
if (num_space <= 8) {
    ...
    space = mx_strlen(string) - num_space;
...
if (num_space > 8) {
    ...
    space = mx_strlen(string) - num_space;  //номер символа первого пробела

    while((num_space % 8) != 0)
        num_space++;
    int num_tab = num_space/8;  //kol tab
